-- Production-safe additive migration for BizPlay conversational schema, v2.
-- Handles older production tables using created_at/updated_at while current code expects created_date/updated_date.
...
-- -----------------------------------------------------------------------------
-- Create missing tables. Existing tables are left intact.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Add missing columns to older production table versions.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Backfill current column names from older created_at/updated_at/proof_date names when present.
-- Dynamic SQL is required because direct references to missing columns fail parse-time validation.
-- -----------------------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_department' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_department SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_staff' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_staff SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_agent_session' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_agent_session SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_agent_session' AND column_name = 'updated_at') THEN
        EXECUTE 'UPDATE conversational_agent_session SET updated_date = COALESCE(updated_date, updated_at) WHERE updated_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_trip_plan' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_trip_plan SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_trip_plan' AND column_name = 'updated_at') THEN
        EXECUTE 'UPDATE conversational_trip_plan SET updated_date = COALESCE(updated_date, updated_at) WHERE updated_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_traveler' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_traveler SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_cost_expense' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_cost_expense SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_cost_expense' AND column_name = 'proof_date') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_transportation_expense' AND column_name = 'created_at') THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_transportation_expense' AND column_name = 'proof_date') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_trip_report' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_trip_report SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'conversational_attachment' AND column_name = 'created_at') THEN
        EXECUTE 'UPDATE conversational_attachment SET created_date = COALESCE(created_date, created_at) WHERE created_date IS NULL';
    END IF;
END $$;

-- Backfill defaults for nullable rows.
...
-- Tighten NOT NULL only for safe columns that have defaults/backfills.
...
-- -----------------------------------------------------------------------------
-- Constraints for older existing tables. Guarded to avoid duplicates.
-- -----------------------------------------------------------------------------
DO $$
BEGIN
